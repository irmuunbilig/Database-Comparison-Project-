...
class Neo4jImporter:
    ...
    def create_indexes(self):
        with self.driver.session() as session:
            session.run("CREATE INDEX IF NOT EXISTS FOR (p:Professor3) ON (p.prof_id)")
            session.run("CREATE INDEX IF NOT EXISTS FOR (c:Course3) ON (c.course_id)")
            session.run("CREATE INDEX IF NOT EXISTS FOR (s:Schedule3) ON (s.course_id)")
    ...
    def _create_professor(tx, batch):
        query = """
        UNWIND $rows AS row
        CREATE (p:Professor3 {prof_id: toInteger(row.prof_id), prof_name: row.prof_name, major: row.major})
        """
    ...
    def _create_schedule(tx, batch):
        query = """
        UNWIND $rows AS row
        CREATE (s:Schedule3 {course_id: toInteger(row.course_id), semester: row.semester, day: row.day, start_time: row.start_time, end_time: row.end_time})
        """
        tx.run(query, rows=batch)
    ...
    def _create_course(tx, batch):
        query = """
        UNWIND $rows AS row
        CREATE (c:Course3 {course_id: toInteger(row.course_id), course_name: row.course_name, prof_id: toInteger(row.prof_id), credit_num: toInteger(row.credit_num), campus: row.campus})
        """
        tx.run(query, rows=batch)
    ...
    def _create_relationships(tx, batch):
        query = """
        UNWIND $rows AS row
        MATCH (p:Professor3 {prof_id: toInteger(row.prof_id)}), (c:Course3 {course_id: toInteger(row.course_id)})
        CREATE (p)-[:TEACHES]->(c)
        WITH row, c
        MATCH (s:Schedule3 {course_id: toInteger(row.course_id)})
