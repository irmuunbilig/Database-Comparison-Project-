...
# Step 1: Query the schedules table for all course_ids and semesters
query_all_courses = "SELECT course_id, semester FROM schedules ALLOW FILTERING;"
...
# Step 3: Query the courses table for prof_ids and campus for the above course_ids where campus is Papardo
prof_ids_and_campuses = []
for course_id, semester in spring_course_ids:
    query_course = f"SELECT prof_id, campus FROM courses WHERE course_id = {course_id} AND campus = 'Papardo' ALLOW FILTERING;"
...
if not prof_ids_and_campuses:
    ...
    # Step 4: Query the professors table for prof_name using the prof_ids
    query_prof_details = f"SELECT prof_id, prof_name FROM professors WHERE prof_id IN ({prof_ids_str})"
