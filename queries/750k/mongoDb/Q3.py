...
# Step 1: Fetch all schedules3 for Spring semesters
schedule_query = {'semester': {'$regex': '^Spring'}}
schedules = execute_query(database['schedules3'], schedule_query)

# Extract the course IDs from the schedules
course_ids = {schedule['course_id'] for schedule in schedules}

# Step 2: Fetch courses3 taught at Papardo Campus with the extracted course_ids
course_query = {'campus': 'Papardo', 'course_id': {'$in': list(course_ids)}}
courses = execute_query(database['courses3'], course_query)

# Extract the professor IDs from the courses
prof_ids = {course['prof_id'] for course in courses}

# Step 3: Fetch the names of professors3 with the extracted prof_ids
professor_query = {'prof_id': {'$in': list(prof_ids)}}
professors = execute_query(database['professors3'], professor_query)
